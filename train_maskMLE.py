...
parser.add_argument('--no_pretrained',action="store_false",required=False,help='(default=%(default)d')
...
def create_ptb_loader(data_dir, batch_size, seq_len):
    ...
    train_x, train_y = ptb_batcher(train_data, seq_len)
    train_loader = ptb_loader(train_x , train_y, batch_size)
    test_x, test_y = ptb_batcher(test_data, seq_len)
    test_loader = ptb_loader(test_x , test_y, batch_size)
    valid_x, valid_y = ptb_batcher(valid_data, seq_len)
...
def main():
    ...
    if not args.no_pretrained:
        model = model.load_pretrained_weights(os.path.join('experiments', args.model_size, 'model_best.pth.tar'))
    else:
