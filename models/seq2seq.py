...
class Seq2Seq(torch.nn.Module):
    ...
    def forward(self,x,h, teacher_forcing_ratio=0.5):
        ...
        max_len = x.shape[-1]
        predictions = torch.zeros(self.config.batch_size, max_len, self.config.vocab_size)
        y, (enc_h,enc_c) = self.encoder(x,h) # z = context vector
        ...
        for t in range(1, max_len):
            output, attention_weights = self.decoder(input.long(),(enc_h, enc_c), y)
        ...
        return predictions, (enc_h, enc_c)
        
    
    def load_pretrained_weights(self, path):
        checkpoint = torch.load(path)
        self.encoder.load_state_dict(checkpoint['state_dict'], strict=False)
        self.decoder.load_state_dict(checkpoint['state_dict'], strict=False)
    
    def get_initial_states(self):
        # Set initial hidden and memory states to 0
        return (torch.zeros(self.config.num_layers,self.config.batch_size,self.config.hidden_size),
                torch.zeros(self.config.num_layers,self.config.batch_size,self.config.hidden_size))
